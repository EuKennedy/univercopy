...
const STEPS = ['Conteúdo', 'Estilo', 'Contexto'] as const

function friendlyError(code: string, raw: string): string {
  switch (code) {
    case 'ai_failed': return 'A IA não respondeu agora. Tente de novo em instantes.'
    case 'feature_locked': return 'Esse recurso não está no seu plano atual.'
    case 'cap_reached': return 'Você atingiu o limite de uso de IA do mês.'
    default: return raw?.length > 160 ? 'Não foi possível gerar agora. Tente novamente.' : raw
  }
}
...
export function GeneratorForm({ slug, pieceTypes, styles, frameworks, categories, products, campaigns, initialProductId = '' }: Props) {
  ...
  async function onGenerate() {
    ...
    if (res.data.variations.length === 0) setError('A IA não retornou variações. Refine o brief e tente de novo.')
  }
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-[var(--uc-text-muted)]">Resultado</p>
            <h2 className="text-xl font-bold text-[var(--uc-text)] mt-1">{variations.length > 0 ? `${variations.length} variações` : 'Geração'}</h2>
          </div>
          <GlassButton variant="secondary" onClick={() => { setDone(false); setError(null) }}>
            <Icon name="chevron-left" size={16} />Ajustar e gerar de novo
          </GlassButton>
        </div>

        {loading && <LoadingCard n={n} />}
        {error && <ErrorBox paywall={paywall} text={error} />}
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {step === 0 && (
        <GlassCard className="p-6 space-y-5">
          <Header title="O que você quer criar?" sub="Escolha o formato do conteúdo. A IA adapta com o DNA da marca." />
      ...
      {step === 1 && (
        <GlassCard className="p-6 space-y-5">
          <Header title="Estilo da escrita" sub="Opcional. Cada estilo é um copywriter lendário com uma assinatura própria." />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <SelectCard active={styleKey === ''} title="Deixar a IA escolher" sub="Auto — melhor estilo para o formato" onClick={() => setStyleKey('')} />
          </div>
      ...
      {step === 2 && (
        <GlassCard className="p-6 space-y-5">
          <Header title="Contexto & brief" sub="Quanto mais específico, melhor a copy." />

          {(products.length > 0 || campaigns.length > 0) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {products.length > 0 && (
                <div>
                  <label className="text-xs font-semibold tracking-wide uppercase text-[var(--uc-text-muted)] mb-1.5 block">Produto da loja <span className="normal-case text-[var(--uc-text-faint)]">(opcional)</span></label>
              ...
              {campaigns.length > 0 && (
                <div>
                  <label className="text-xs font-semibold tracking-wide uppercase text-[var(--uc-text-muted)] mb-1.5 block">Campanha <span className="normal-case text-[var(--uc-text-faint)]">(opcional)</span></label>
          ...
          <div>
            <label className="text-xs font-semibold tracking-wide uppercase text-[var(--uc-text-muted)] mb-1.5 block">Brief</label>
            <textarea rows={5} className={cn(fieldCls, 'py-3 resize-y leading-7')} placeholder="Objetivo, oferta, ângulo, contexto…" value={brief} onChange={(e) => setBrief(e.target.value)} />
          </div>

          <div className="grid grid-cols-2 gap-4 max-w-sm">
            <div>
              <label className="text-xs font-semibold tracking-wide uppercase text-[var(--uc-text-muted)] mb-1.5 block">Variações</label>
              <select className={cn(fieldCls, 'h-12')} value={n} onChange={(e) => setN(Number(e.target.value))}>
                {[1, 2, 3, 4, 5].map((v) => <option key={v} value={v}>{v}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-semibold tracking-wide uppercase text-[var(--uc-text-muted)] mb-1.5 block">Modelo</label>
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            <Chip label="Conteúdo" value={selectedPiece?.name ?? '—'} />
            <Chip label="Estilo" value={selectedStyle ? selectedStyle.name : 'Auto'} />
          </div>

          <div className="flex justify-between items-center pt-1">
            <GlassButton variant="ghost" onClick={() => setStep(1)}><Icon name="chevron-left" size={16} />Voltar</GlassButton>
            <GlassButton size="lg" loading={loading} disabled={!pieceTypeKey} onClick={onGenerate}>
              {!loading && <Icon name="spark" size={18} />}{loading ? 'Gerando…' : 'Gerar copy'}
            </GlassButton>
          </div>
        </GlassCard>
      )}
...
function LoadingCard({ n }: { n: number }) {
  return (
...
function ErrorBox({ paywall, text }: { paywall: boolean; text: string }) {
  return (
    <div className={cn(
      ...
      {paywall ? 'Limite do plano atingido. ' : ''}{text}
    </div>
  )
}
    ...
    <GlassCard className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-[var(--uc-text-muted)]">Variação {index + 1}</p>
